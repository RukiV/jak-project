...
## Process on this mirror

Upstream's own agent guide tells contributors never to open an issue or a pull request. That
rule is about contributing to the upstream project. On this staging mirror issues and pull
requests are the working record and are expected.
...
Contributors disclose AI involvement with `(AI-assisted)` in the commit message. The rule
binds contributors, not the owner of this forge: CI checks every non-merge commit whose
author is not the maintainer, and maintainer-authored commits are exempt by design rather
than by oversight. Work offered upstream follows upstream's expectations, not this one.
